...
const before = await page.evaluate(() => JSON.stringify(cachedLayout.glyphs.map(g => g.strokes[0][0])));
...
const result = await page.evaluate(() => ({
  charOff: JSON.stringify(S.charOff),
  lineOff: JSON.stringify(S.lineOff),
  textX: S.textX,
}));
const after = await page.evaluate(() => JSON.stringify(cachedLayout.glyphs.map(g => g.strokes[0][0])));
